...
const validateSession = async (req: Request, res: Response) => {
    ...
    if(response !== null) {
        if (response) {
            ...
            if (updatedSession) {
                return res.cookie('sessionCookie', {'sessionId': sessionId, 'expires': updatedSession.sessionExpiry, 'domain': 'iot-dashboard-backend-server.onrender.com'}, {'sameSite': 'none', 'secure': true}).status(200).json({ message: 'Session is not expired', user: response });
            } else {
